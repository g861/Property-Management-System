...
public class UpdateProperty {
	...
	public void updateProperty(Connection conn, Scanner sc, BufferedReader br) {
		try {
			...
			while (true) {
				...
				sc.nextLine();
				Boolean res = checkValidRows(propertyIdToUpdate, conn);
				if (res == false) {
					System.out.println(" Warnning.....enter valid  ID.");
					continue;
				}
				String updateQuery;
	...
	// Check Valid Row
	private boolean checkValidRows(int pid, Connection con) throws SQLException {
		String query = "SELECT * FROM property where Pid=" + pid + ";";
